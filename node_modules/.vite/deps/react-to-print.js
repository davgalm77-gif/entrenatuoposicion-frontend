...
//#region node_modules/react-to-print/dist/react-to-print.mjs
...
function P(e) {
	return e instanceof Error ? e : /* @__PURE__ */ new Error("Unknown Error");
}
function H(e, t) {
	const { documentTitle: n, onAfterPrint: l, onPrintError: p, preserveAfterPrint: m, print: h, suppressErrors: g } = t;
	setTimeout(() => {
		if (e.contentWindow) {
			let a = function() {
				l?.(), $(m);
			};
			if (e.contentWindow.focus(), h) h(e).then(a).catch((c) => {
				p ? p("print", P(c)) : f({
...
function z() {
	return [
		...
		/BlackBerry/i,
		/Windows Phone/i
	].some((t) => (navigator.userAgent ?? navigator.vendor ?? ("opera" in window && window.opera)).match(t));
}
function O(e) {
	const t = [], n = document.createTreeWalker(e, NodeFilter.SHOW_ELEMENT, null);
	let l = n.nextNode();
	for (; l;) t.push(l), l = n.nextNode();
	return t;
}
function j(e, t, n) {
	const l = O(e), p = O(t);
...
`;
function W(e, t, n) {
	const { contentNode: l, clonedContentNode: p, clonedImgNodes: m, clonedVideoNodes: h, numResourcesToLoad: g, originalCanvasNodes: a } = t, { bodyClass: c, fonts: E, ignoreGlobalStyles: y, pageStyle: C, nonce: T, suppressErrors: A, copyShadowRoots: F } = n, L = [], _ = [];
	function i(k, x) {
		if (L.includes(k)) {
			f({
				level: "debug",
		...
		x ? (f({
			messages: ["\"react-to-print\" was unable to load a resource but will continue attempting to print the page", ...x],
			suppressErrors: A
	...
	if (d) {
		...
		F && j(l, k, !!A), E && (e.contentDocument?.fonts && e.contentWindow?.FontFace ? E.forEach((s) => {
			const o = new FontFace(s.family, s.source, {
				weight: s.weight,
				style: s.style
			});
		...
		for (let s = 0; s < m.length; s++) {
			...
			if (!b) i(o, ["Found an <img> tag with an empty \"src\" attribute. This prevents pre-loading it.", o]);
			else {
				const r = new Image();
				r.onload = () => {
					i(o);
		...
		for (let s = 0; s < h.length; s++) {
			...
			} else o.readyState >= 2 ? i(o) : o.src ? (o.onloadeddata = () => {
				i(o);
			}, o.onerror = (r, u, w, S, v) => {
				i(o, [
					"Error loading video",
		...
		for (let s = 0; s < M.length; s++) B[s].value = M[s].value;
		if (!y) {
				...
				if (r.tagName.toLowerCase() === "style") {
					...
					if (w) {
						...
						try {
							...
							for (let N = 0; N < v; ++N) typeof w.cssRules[N].cssText == "string" && (S += `${w.cssRules[N].cssText}\r
`);
						} catch (v) {
							f({
								messages: [
				...
				} else if (r.getAttribute("href")) if (r.hasAttribute("disabled")) f({
...
function K({ contentRef: e, optionalContent: t, suppressErrors: n }) {
	if (t && typeof t == "function") return e && f({
		level: "warning",
...
function Q(e, t) {
	const { contentRef: n, fonts: l, ignoreGlobalStyles: p, suppressErrors: m } = t, h = K({
		contentRef: n,
		optionalContent: e,
		suppressErrors: m
	});
	if (!h) return;
	const g = h.cloneNode(!0), a = document.querySelectorAll("link[rel~='stylesheet'], link[as='style']"), c = g.querySelectorAll("img"), E = g.querySelectorAll("video"), y = l ? l.length : 0;
	return {
		contentNode: h,
		clonedContentNode: g,
		clonedImgNodes: c,
		clonedVideoNodes: E,
		numResourcesToLoad: (p ? 0 : a.length) + c.length + E.length + y,
		originalCanvasNodes: h.querySelectorAll("canvas")
	};
}
function Z({ bodyClass: e, contentRef: t, copyShadowRoots: n, documentTitle: l, fonts: p, ignoreGlobalStyles: m, nonce: h, onAfterPrint: g, onBeforePrint: a, onPrintError: c, pageStyle: E, preserveAfterPrint: y, print: C, printIframeProps: T, suppressErrors: A }) {
